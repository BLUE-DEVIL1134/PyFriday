...
Please Email Me AT akashpattnaik.github@gmail.com
"""
...
__all__ = ["loop"]
__PreRequests__ = ['pyttsx3', 'speech_recognition', 'wikipedia', 'webbrowser', 'pipwin']

"""
This Is Friday Made By Tony Stark,
We Just Made A Replica Of This...
In Pure Python..
"""
...
os.system('pipwin install pyaudio && cls')  # To Make Sure That This Is Installed
...
def _voice_command():
    ...
    with sr.Microphone() as source:
        print('Speak Something --- And Wait...')
        r.pause_threshold = 1
...
def Friday():
    ...
    if 'wikipedia' in query:
        Speak(f'Searching Wikipedia...{boss}')
        query = query.replace("wikipedia", "")
        try:
            results = wikipedia.summary(query, sentences=2)
            Speak(f"According to Wikipedia : {results}")
        except FileNotFoundError:
            Speak(f'Wikipedia Says No Such Page Is Found {boss}')
            return
        ...
        Speak(f'{boss} , Opening Stack OverFlow')
        ...
        Speak(f'{boss}, The Time Is {datetime.datetime.now().strftime("%H:%M:%S")}')
    elif 'date' in query:
        Speak(f'{boss}, The Date Is {datetime.datetime.now().strftime("%d-%m-%y")}')
...
def loop():
    Speak('Friday Started Successfully...')
    while True:
        Friday()
